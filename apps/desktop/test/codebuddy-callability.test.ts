...
test("CodeBuddy CN Desktop stays detected but non-callable without CodeBuddy Code CLI", async () => {
  ...
  assert.equal(qualification.adapter, null);
});
...
test("CodeBuddy qualification registers only after its controlled login probe passes", async () => {
  ...
  assert.ok(qualification.adapter);
  assert.equal(qualification.adapter.entrypoint, "/opt/homebrew/bin/codebuddy");
});

test("CodeBuddy Adapter fixes stdin-only, no-tools, no-Hook, no-MCP, ephemeral execution", () => {
  const adapter = new CodeBuddyCallableAdapter("/opt/homebrew/bin/codebuddy");
