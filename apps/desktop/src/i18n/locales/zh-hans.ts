...
export const ZH_HANS_MESSAGES = {
  ...
  tasks: {
    ...
    longHorizon: {
      ...
      structuredMemory: {
        title: "可选：协作记忆",
        automaticNote: "默认不使用，任务可以直接开始。只有你主动整理并选择后，下一阶段才会参考本地保存的内容。",
        collapsedOff: "默认关闭",
        collapsedOn: "下一阶段将参考 {count} 条",
        collapsedUnavailable: "暂不可用，不影响任务",
        loading: "正在读取记忆状态…",
        unavailable: "协作记忆暂不可用，但不影响批准、执行或保存阶段结果。",
        readOnly: "当前只能查看已记录的 {count} 个阶段，不能新增或用于下一阶段。",
        ready: "已记录 {count} 个阶段结果",
        stage: "阶段 {stage} · {agent}",
        prepare: "整理为记忆",
        edit: "编辑",
        delete: "删除",
