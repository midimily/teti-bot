...
export const ZH_HANS_MESSAGES = {
  ...
  shell: {
    ...
    openTeti: "打开 Teti",
    openPendingTasks: { other: "打开 Teti 任务，# 个任务有新动态" },
  ...
  tasks: {
    ...
    inbox: {
      ...
      singleStage: "单次调用",
      longHorizon: "持续协作",
      longHorizonStage: "持续协作 · 第 {stage} 阶段"
    },
    composer: {
      peer: "发送给",
      capability: "调用能力",
    ...
    detail: {
      ...
      latestChangeTitle: "最新变化",
      latestChangeStage: "{change} · 阶段 {stage}",
      latestChangeAt: "{change} · {date}",
      latestChanges: {
        status_updated: "任务状态已更新",
        delivery_failed: "消息暂未送达，将自动重试",
        delivery_recovered: "先前未送达的消息现已送达",
        input_received: "已收到补充指令",
        stage_started: "阶段已开始",
        stage_completed: "阶段已完成",
        stage_failed: "阶段执行失败",
        pause_requested: "已设置在阶段结束后暂停",
        cancel_requested: "已发起取消",
        paused: "持续协作已暂停",
        resumed: "持续协作已恢复",
        renewed: "持续协作已续期",
        completed: "任务已完成",
      ...
      authorization: {
        ...
        onceTitle: "仅允许一次",
        ongoingLoginTitle: "Agent 登录后 · 开始持续协作",
        ongoingAgentTitle: "{agent} · 持续协作",
        ongoingTitle: "持续协作",
        ongoingDetail: "接受后进入本次持续协作；对方可以继续发送后续阶段指令，每个新阶段仍由你确认后执行，直到任务完成、你主动停止或协作到期。不会获得其他任务的权限。",
      ...
      actions: {
        ...
        allowOnce: "仅允许一次",
        retryOngoingAfterLogin: "登录后接受协作",
        allowOngoing: "接受持续协作",
    ...
    longHorizon: {
      ...
      structuredMemory: {
        title: "协作记忆",
        automaticNote: "已自动开启，下一阶段会参考当前任务已完成的阶段结果。",
        collapsedOff: "默认关闭",
        collapsedOn: "已开启 · {count} 个阶段",
        collapsedUnavailable: "本地保存暂不可用",
        loading: "正在读取记忆状态…",
        unavailable: "当前任务仍会参考已完成阶段；本地记忆保存暂不可用。",
        readOnly: "已保留 {count} 个阶段结果；当前为只读状态。",
        ready: "已在本机保存 {count} 个阶段结果",
        currentTaskOnly: "仅用于当前持续协作任务，不会自动读取其他任务、工作区或协作对端的历史内容。",
        stage: "阶段 {stage}",
        prepare: "整理为记忆",
        edit: "编辑",
        delete: "删除",
        confirmDelete: "确认删除",
        deleteWarning: "删除会立即移除正文、搜索索引和后续注入资格，仅保留不含正文的审计。",
        cancel: "取消",
        save: "确认保存",
        editorTitle: "结构化记忆编辑器",
        titleField: "标题",
        contentField: "作为参考数据保存的正文",
        scopeField: "适用范围",
        kindField: "类型",
        pinned: "置顶参与排序",
        expiryField: "到期时间（可选）",
        expiryHint: "留空表示不会自动到期；到期后立即停止检索，并在本地维护时清理正文。",
        scopesTitle: "可参考哪些内容",
        scopes: { task: "本任务", workspace: "本工作区", peer: "该协作对端" },
        scopeEnabled: "已允许 · {count} 条",
        scopeDisabled: "未允许 · {count} 条",
        candidatesTitle: "下一阶段可参考的内容",
        candidatesEmpty: "目前没有可用内容。完成一个阶段后，可先整理结果再选择使用。",
        temporaryExclude: "本次包含；取消勾选可临时排除",
        budgetExcluded: "未纳入：已达到候选或字节预算",
        injectNext: "让下一阶段参考以上内容",
        injectNote: "只使用一次；内容变化或预览失效时，将自动按不使用记忆执行。",
        refreshPreview: "刷新预览",
        previewSummary: "已选 {count} 条 · {bytes} 字节 · 执行 Agent {agent}",
        lastInjection: "上一次执行参考了 {count} 条内容",
        kinds: {
          decision: "决策",
          constraint: "约束",
          fact: "事实",
        ...
        errors: {
          read_failed: "记忆预览暂不可用；不影响批准或继续任务。",
          write_failed: "记忆操作未完成；任务本身仍可无记忆执行。",
          preview_stale: "预览已过期或与当前 Child/内容不一致；本次将无记忆执行，请刷新后重新勾选。"
        }
      },
      phase: {
      ...
      instructionPending: "已发送，等待对端开始下一阶段",
      stageLimitReached: "已达到 16 个阶段上限，请等待对端确认当前结果并完成任务。",
      ...
      renewing: "正在续期…",
      renewedUntil: "已延长 1 小时，新的截止时间：{date}",
      renewFailed: "未能续期。可能已达到续期次数或最长协作时限，当前截止时间没有变化。",
