...
export const ZH_HANS_MESSAGES = {
  ...
  tasks: {
    ...
    longHorizon: {
      ...
      structuredMemory: {
        title: "本地持续协作记忆",
        automaticNote: "阶段结果先进入 SQLite 草稿源；只有你确认并编辑为结构化条目后，才可能进入执行前预览。旧版 Memory 不参与。",
        loading: "正在读取本地记忆状态…",
        unavailable: "本地记忆库暂不可用；任务结果仍保留，Runtime 会在后续读取时重试。",
        readOnly: "SQLite 处于安全只读模式；可查看 {count} 个阶段，但不会写入或注入。",
        ...
        scopesTitle: "精确授权范围",
        scopes: { task: "当前任务", workspace: "当前 Workspace", peer: "当前协作对端" },
        scopeEnabled: "已开启 · {count} 条可用",
        scopeDisabled: "默认关闭 · {count} 条候选",
        candidatesTitle: "下一次执行的只读预览",
        candidatesEmpty: "当前没有可注入条目。可先整理阶段结果，或开启精确范围授权。",
        temporaryExclude: "本次包含；取消勾选可临时排除",
        budgetExcluded: "未纳入：已达到候选或字节预算",
        injectNext: "仅为下一次执行注入以上条目",
        injectNote: "默认关闭；批准后只消费一次。预览过期、编辑、删除或授权变化都会自动降级为无记忆执行。",
        refreshPreview: "刷新预览",
        previewSummary: "将注入 {count} 条 · {bytes} 字节 · Child {agent}",
        lastInjection: "最近一次已注入 {count} 条（本地审计）",
        kinds: {
          decision: "决策",
          constraint: "约束",
          fact: "事实",
